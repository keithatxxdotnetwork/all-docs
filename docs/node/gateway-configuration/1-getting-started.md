...
* **Gateway PostgreSQL Database Username**: This username is `cmix` by default and is set in the `gateway.yaml` file.
* **Gateway PostgreSQL Database Password**: This password will be set in a later step, [Create the PostgreSQL user named `cmix`](gateway-database#create-the-postgresql-user-named-cmix), and should be different than the sudo-enabled Account password.
* **Private and Public IP Addresses**: If you do not know these you can, [Get Public and Private IP addresses](../references/basic-linux-help.md#get-public-and-private-ip-addresses) of the computer.
