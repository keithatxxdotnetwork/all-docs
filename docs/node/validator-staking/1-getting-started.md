...
* **Validator Controller Account**: This account is akin to a hot wallet which will require a small amount of xx coins for the purpose of paying transaction fees related to the stash account for signing an extrinsic such as initiating payouts and changes to the validator commission.
...
* **cMix Hexadecimal Node ID**: The hexadecimal node ID can be [gotten from the `cmix-IDF.json'](../../node/references/basic-linux-help.md#get-cmixid-and-hexnodeid-from-cmix-idfjson) file and instructions will also be provided at a later step as well.
