...
governance strives to balance the will of the token holders with the
...
The general flow of the network is that the Token Holders elect the
Council, and the Council elects the Technical Committee. Thus, the Token
Holders are the root of all authority within the xx network.

Token Holders use their coins to nominate nodes, pass referenda, and
elect the Council.

are configurable and subject to change before release or by governance.
...
1.  **Public Referenda:** Referenda submitted and propagated by token
    holders
...
in a fixed time window that will be, on network launch, . All referenda
...
referenda, the enactment period is .
...
deposits a number of tokens of at least xx. Then, any other coin holder
...
The public proposal queue has a maximum size of proposals.
...
period of . After this period, the same referendum can be resubmitted.
...
referendum enters a fast-track voting period with a minimum of . If
unanimity is achieved, a voting period smaller than is possible.
...
A supermajority of the Council can cancel any active referendum. This is
...
Council members in each period. These terms will last . The election
selects the 13 Council members and the , which keep their votes in the
election.

The Council has the power to propose referenda (see [Council
Referenda](#Council_Referenda "wikilink")), control the treasury, cancel
...
staking ratio). Any token holder may make proposals to the Council to
access funds by staking a minimum of of the request, or 100 coins,
whichever is higher. The stake is slashed if the proposal is rejected
and returned if it is accepted.

To accept a proposal, at least of the Council must support it. The
threshold to reject a proposal is a simple majority.

If there are unspent funds at the end of a treasury period (), a portion
will be burned. At network launch, this burn rate will be set at .

Along with the proposal system, any token holder can suggest a tip. Any
...
deposit, and the suggester receives of the total tip award.
...
(as described in the [council referendum
section](#Council_Referenda "wikilink")), and the ability to fast-track
